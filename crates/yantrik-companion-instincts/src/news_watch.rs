        ...
        let execute_msg = match state.model_tier {
            ModelTier::Large => format!(
                "EXECUTE Current awareness lens: {lens_name}.\n\
                 Step 1: Call recall with query \"{lens_name} news update\" to check what you already \
                 reported recently. If you already shared a {lens_name} update today, respond with \
                 just \"No world update.\"\n\
                 Step 2: Use web_search to search for \"{search_query}\".\n\
                 {filter}\n\
                 {interests}{location}\n\
                 ANALYSIS: Do NOT just repeat headlines. THINK about what you found.\n\
                 - Explain second-order effects: how does this ripple into {user}'s world?\n\
                 - Look for the NON-OBVIOUS angle that a generic news summary would miss.\n\
                 Deliver ONE significant finding in 2-3 analyzed sentences.\n\
                 If nothing significant, just say \"No world update.\"\n\
            ...
            _ => format!(
                "EXECUTE Check what's happening in the world through a {lens_name} lens. \
                 First recall what {lens_name} news you already shared with {user} recently — don't repeat yourself. \
                 Then search for \"{search_query}\". \
                 {filter}\
                 {interests}{location}\n\
                 Don't just repeat headlines — explain second-order effects on {user}'s world. \
                 Deliver ONE significant finding in 2-3 analyzed sentences. \
                 If nothing significant, just say \"No world update.\" \
