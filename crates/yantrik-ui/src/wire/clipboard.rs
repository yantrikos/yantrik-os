//! Wire clipboard history panel — populate entries, handle paste.
...
/// Build the model from SharedHistory and push it to the UI.
fn populate_entries(ui: &App, clip: &SharedHistory) {
    let history = clip.lock().unwrap();
    let entries: Vec<ClipboardEntryData> = history
        .recent(20)
        .iter()
        .enumerate()
