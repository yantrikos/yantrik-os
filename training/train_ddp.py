#!/usr/bin/env python3
"""DDP multi-GPU fine-tuning of Qwen3.5-4B for Yantrik companion.

Uses unsloth + torchrun for 2x RTX 3090 Ti via WSL2.
Run: torchrun --nproc_per_node=2 training/train_ddp.py
"""
...
# Do NOT set CUDA_VISIBLE_DEVICES — let torchrun handle it
...
def main():
    ...
    if is_main:
        print(f"\n{'='*60}")
        print(f"Loading {MODEL_NAME} in bf16 (DDP mode, 2 GPUs)...")
        print(f"{'='*60}\n")
    ...
    effective_batch = args.batch_size * GRAD_ACCUM_STEPS * n_gpus
    total_steps = (len(train_dataset) * args.epochs) // effective_batch
    if is_main:
        print(f"\nGPUs: {n_gpus}, Batch: {args.batch_size} x {GRAD_ACCUM_STEPS} x {n_gpus} = {effective_batch}")
    ...
    trainer = SFTTrainer(
        ...
        args=SFTConfig(
            ...
            per_device_eval_batch_size=2,  # Lower eval batch to avoid OOM
